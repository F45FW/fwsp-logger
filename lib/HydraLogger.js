...
class HydraLogger extends HydraPlugin {
   ...
   */
  onServiceReady() {
    this.hydra.on('log', (function(entry) {
      let _entry = entry;
      if (typeof(entry.msg) === 'object') {
        if (entry.msg instanceof Error) {
          _entry = Object.assign(entry, this._logger.errSerializer(entry.msg));
        } else {
          _entry = Object.assign(entry, entry.msg);
        }
      }

      this._logger.pino[_entry.type.toLowerCase()](_entry);
    }).bind(this));
  }
}
